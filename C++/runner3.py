##go to directory with c++ code, call C:\python27\python runner3.py exec tests
##1.in 1.out
...
files.sort(key=lambda x:(int( os.path.splitext(x)[0] ), x))
N = len(files)
...
for f_name in files:
    if os.path.splitext(f_name)[1] == '.in':
        ...
        print "Test Case " + (os.path.splitext(f_name)[0])+":",
        ...
        if x is None:
            ...
            fline = fcor.readline()
            print "Expected Output: " + fline.strip()#+"\n"
