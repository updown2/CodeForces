##go to directory with c++ code, call C:\python27\python runner.py exec tests
##file.1.in, file.1.out
...
files.sort(key=lambda x:(int( (os.path.splitext(os.path.splitext(x)[0])[1])[1:] ), x))
N = len(files)
...
for f_name in files:
    if os.path.splitext(f_name)[1] == '.in':
        ...
        print "Test Case " + (os.path.splitext(os.path.splitext(f_name)[0])[1])[1:]+":",
